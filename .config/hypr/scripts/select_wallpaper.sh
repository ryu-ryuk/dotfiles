...
SELECTED_WALLPAPER=$(find "$WALLPAPER_DIR" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" \) \
    -print0 | shuf -z | xargs -0 -I {} echo -en "{}\x00icon\x1f{}\n" | rofi -dmenu -show-icons -theme-str "${r_override}" -config "${RofiConf}")
r_override="element{border-radius:${elem_border}px;} listview{columns:6;spacing:100px;} element{padding:0px;orientation:vertical;} element-icon{size:${x_monres}px;aspect:1;border-radius:0px;} element-text{padding:1em;}"
